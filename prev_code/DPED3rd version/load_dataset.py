...
def load_test_data(phone, dped_dir, IMAGE_SIZE):
    ...
    for i in range(0, NUM_TEST_IMAGES):
        I = misc.imread(test_directory_phone + str(i+12690) + '.jpg')
        ...
        I = misc.imread(test_directory_dslr + str(i+12690) + '.jpg')
        I = np.expand_dims(I, axis=2)
        I = np.concatenate((I, I, I), axis=-1)
        I = np.asarray(I)
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE]))/255
...
def load_batch(phone, dped_dir, TRAIN_SIZE, IMAGE_SIZE):
    ...
    for img in TRAIN_IMAGES:
        I = misc.imread(train_directory_phone + str(img) + '.jpg')
        I = np.expand_dims(I, axis=2)
        I = np.concatenate((I, I, I), axis=-1)
        I = np.asarray(I)
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
        ...
        I = misc.imread(train_directory_dslr + str(img) + '.jpg')
        I = np.expand_dims(I, axis=2)
        I = np.concatenate((I, I, I), axis=-1)
        I = np.asarray(I)
        I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
