    ...
    if phone.endswith("_orig"):
        ...
        for i in iteration:
            ...
            for photo in test_photos:
                ...
                photo_name = photo.rsplit(".", 1)[0]
                print(np.max(enhanced_image))
                enhanced_image = np.uint16(enhanced_image)
                print(np.max(enhanced_image))
                # print(np.max(enhanced_image))
                img.PixelData = enhanced_image.tobytes()
                img.save_as("visual_results/" +photo+ "_enhanced.dcm")
...
with tf.Session(config=config) as sess:
    ...
    # load pre-trained model
    saver = tf.train.Saver()
    saver.restore(sess, "models/" + phone + "_iteration_" + str(iteration) + ".ckpt")

    top_path = dped_dir + phone.replace("_orig", "") + "/test_data/full_size_test_images/k_dcm"
    test_dirs = os.listdir(top_path)
    for test_dir in test_dirs:

        test_photos = [f for f in os.listdir(top_path+'/'+test_dir) if os.path.isfile(top_path+'/'+test_dir +'/'+f)]
        print(test_photos)
        if test_subset == "small":
            # use five first images only
            test_photos = test_photos[0:5]
        ...
        for photo in test_photos:
            ...
            print("iteration " + str(iteration) + ", processing image " + photo)
            img = pydicom.read_file(top_path+'/'+test_dir +'/' + photo)
            imge = img.pixel_array
            # print(np.max(imge))
            # # imge = normal(imge)
            norpro = nomalprocess()
            ...
            # print(np.max(enhanced_image))
            img.PixelData = enhanced_image.tobytes()
            out_path = "visual_results/"+test_dir
            if not os.path.exists(out_path):
                os.mkdir(out_path)
            img.save_as(out_path+'/'+ photo_name + "_enhanced.dcm")
